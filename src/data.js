export const infiniteCarouselSlides = [
  { url: "braclet.png", title: "bracelets" },
  { url: "earrings.png", title: "earrings" },
  { url: "necklace.png", title: "necklaces" },
  { url: "rings.png", title: "rings" },
];

export const carouselSlides = [
  { url: "1.jpg", title: "eclipse" },
  { url: "2.jpg", title: "afterlight" },
  { url: "3.jpg", title: "stillnes" },
  { url: "4.jpg", title: "undertone" },
];

export const cards = [
  {
    img: "ring1.png",
    title: "Lunar Fold",
    material: "Sterling Silver",
    price: `$${380}`,
  },
  {
    img: "ring1.png",
    title: "Lunar Fold",
    material: "Sterling Silver",
    price: `$${380}`,
  },
  {
    img: "ring1.png",
    title: "Lunar Fold",
    material: "Sterling Silver",
    price: `$${380}`,
  },
  {
    img: "ring1.png",
    title: "Lunar Fold",
    material: "Sterling Silver",
    price: `$${380}`,
  },
];

export const body = ["ear", "wrist", "finger", "neck"];
export const material = ["silver", "gold", "pearl", "platinum"];
export const collections = ["eclipse", "afterlight", "stillness", "undertone"];

export const comments = [
  {
    comment:
      "An extraordinary balance of bold design and delicate detail. These creations elevate jewelry from adornment to art",
    author: "VÉRITÉ",
  },
  {
    comment:
      "Striking, confident, and unapologetically modern. Each piece tells a story of power and individuality",
    author: "LUMEN HOUSE",
  },
  {
    comment: "LOUNARIA understands restraint. The silver speaks without excess",
    author: "Rêverie house",
  },
  {
    comment:
      "These creations blur the boundary between wearable object and contemporary art",
    author: "SEIRU",
  },
];

export const footerInfo = [
  { category: "contact us", link: "" },
  { category: "faq", link: "" },
  { category: "legal terms", link: "" },
  { category: "privacy policy", link: "" },
];
